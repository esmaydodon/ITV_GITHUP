...
//hace algo     
 $(".gestionarVehiculo").click(function(evento)
      {
          $("#listarVehiculo").css("display", "block");
          $("#editVehiculo").css("display", "block");
          $("#formulario-vehiculo-nuevo").css("display", "none");
}); 
//hace algo end 
...
Modelo: <?php
 $consulata_tipo2=dime('select * from  modelo   where estado_modelo = 1  order by  idmodelo desc') or die(mysql_error());	
    echo"<select id ='modelo' name='modelo' class='select'>
    	...
    	echo"<option value='".$local['idmodelo']."'>".$local['nombre_modelo']."</option>";}
	echo"</select></br>"; 
        ?>

Tipo: <?php
 $consulata_tipo2=dime('select * from  tipo  where estado_tipo = 1 order by  idtipo desc') or die(mysql_error());	
    echo"<select id ='tipo' name='tipo' class='select'>
	     <option value='0'>Seleccione tipo</option>";
		while($local = mysql_fetch_array($consulata_tipo2)){
    	echo"<option value='".$local['idtipo']."'>".$local['descripcion_tipo']."</option>";}
	echo"</select></br>"; 
        ?>
