...
<li><a href="itv.php" >Registrar Itv</a></li>
 ...
 <form action="RegistrandoObservacion.php" method="post" enctype="multipart/form-data" onSubmit="return checkForm(this);" >
     Nombre Observacion :<spam class="requerido" >*</spam><input type="text" onBlur="checkField(this);" name="descripcion_observacion"  id="descripcion_observacion" bsize="60" maxlength="120" value="" /> <br>

       Pertenece : <?php
 $consulata_tipo2=dime('select * from  observacion where estado_observacion = 1 AND idsub_observacion=0 order by  idobservacion desc') or die(mysql_error());	
    echo"<select id ='observacion' name='observacion' class='select'>
	     <option value='0'>Seleccione marca</option>";
		while($local = mysql_fetch_array($consulata_tipo2)){
    	echo"<option value='".$local['idobservacion']."'>".$local['descripcion_observacion']."</option>";}
	echo"</select></br>"; 
        ?>  
